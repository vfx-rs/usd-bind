...
{
    try {
        to_c_copy(return_, pxrInternal_v0_21__pxrReserved__::UsdStage::CreateNew(to_cpp_ref(identifier), pxr_UsdStage_InitialLoadSet_to_cpp_ref(&(load))));
        return 0;
...
{
    try {
        to_c_copy(return_, pxrInternal_v0_21__pxrReserved__::UsdStage::CreateInMemory(pxr_UsdStage_InitialLoadSet_to_cpp_ref(&(load))));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
