#include "pxr/usd/sdf/c-layer_private.h"
...
{
    try {
        *(return_) = pxrInternal_v0_20__pxrReserved__::SdfLayer::IsAnonymousLayerIdentifier(to_cpp_ref(identifier));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
...
{
    try {
        to_c_copy(return_, pxrInternal_v0_20__pxrReserved__::SdfLayer::GetDisplayNameFromIdentifier(to_cpp_ref(identifier)));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
...
{
    try {
        to_c_copy(return_, pxrInternal_v0_20__pxrReserved__::SdfLayer::ComputeRealPath(to_cpp_ref(layerPath)));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
