...
{
    try {
        to_c_copy(return_, pxrInternal_v0_21__pxrReserved__::UsdGeomSphere::Define(to_cpp_ref(stage), to_cpp_ref(path)));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
...
{
    try {
        to_c(this_, new pxrInternal_v0_21__pxrReserved__::UsdGeomSphere(to_cpp_ref(rhs)));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
