#include "pxr/usd/usd_geom/usd_geom_xform_private.h"
...
{
    try {
        to_c_copy(return_, pxrInternal_v0_21__pxrReserved__::UsdGeomXform::Define(to_cpp_ref(stage), to_cpp_ref(path)));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
...
{
    try {
        to_c(this_, new pxrInternal_v0_21__pxrReserved__::UsdGeomXform(to_cpp_ref(rhs)));
        return 0;
    } catch (std::exception& e) {
        TLG_EXCEPTION_STRING = e.what();
        return -1;
