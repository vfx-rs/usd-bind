...
struct SdfFileFormat {
    using BoundType = pxr::SdfFileFormat;
...
template <class T>
struct Sdf_FileFormatFactory {
    using BoundType = pxr::Sdf_FileFormatFactory<T>;

    pxr::SdfFileFormatRefPtr New() const;

} CPPMM_OPAQUEPTR; // struct Sdf_FileFormatFactory

// TODO: fill in explicit instantiations, e.g.:
// template class Sdf_FileFormatFactory<int>;
// using Sdf_FileFormatFactoryInt = pxr::Sdf_FileFormatFactory<int>;


template <typename FileFormat, typename BaseFormats>
