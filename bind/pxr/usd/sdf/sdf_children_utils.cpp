...
struct Sdf_ChildrenUtils {
    ...
    using FieldType = typename pxr::Sdf_ChildrenUtils<ChildPolicy>::FieldType;
    ...
    static pxr::SdfAllowed CanRename(const pxr::SdfSpec& spec, const FieldType& newName);
    ...
    static bool Rename(const pxr::SdfSpec& spec, const FieldType& newName);
    ...
    static bool MoveChildForBatchNamespaceEdit(const pxr::SdfLayerHandle& layer, const pxr::SdfPath& path, const UNKNOWN& value, const FieldType& newName, int index);
    ...
    static bool CanMoveChildForBatchNamespaceEdit(const pxr::SdfLayerHandle& layer, const pxr::SdfPath& path, const UNKNOWN& value, const FieldType& newName, int index, std::string* whyNot);
#endif

    /// Returns \c true if the child of \p path identified by \p key can
    /// be removed, otherwise returns \c false and sets \p whyNot.
    static bool CanRemoveChildForBatchNamespaceEdit(const pxr::SdfLayerHandle& layer, const pxr::SdfPath& path, const FieldType& key, std::string* whyNot);
